# --------------------------------------------------------
# Altered by Tina Jensen - Aarhus University
# built on TensorFlow MobileNetV1 and mobilenet_v1 by:
...
# The deeper MobileNet modification tested (Project report, Experiment 5)
# 1 x 64 + 2 x 128 + 2 x 256 + 6 x 512 + 6 x 1024 depthwise seperable convolutions
# Accuracy was low compared to expected - suspected underfitting due to long thin net
# Conv : initial 3x3 convolution layer
# DepthSepConv : 
# 2 layer building block of one 3x3 depthwise convolution and one 1x1 convolution.
...
# # The deeper MobileNet layout intended to be tested (Project report, Experiment 5)
# # 1 x 64 + 3 x 128 + 3 x 256 + 8 x 512 + 2 x 1024 depthwise seperable convolutions
# # It was not possible to run this configuration due to conflicts with 
# # pre-trained weights dimensions - fixing this is part of future work
# Conv = namedtuple('Conv', ['kernel', 'stride', 'depth'])
# DepthSepConv = namedtuple('DepthSepConv', ['kernel', 'stride', 'depth'])
...
# raise the number of possible fixed layers accordingly to deeper net
  def _image_to_head(self, is_training, reuse=None):
    # Base bottleneck
    assert (0 <= cfg.MOBILENET.FIXED_LAYERS <= 16)
    ...
    if cfg.MOBILENET.FIXED_LAYERS > 0:
      with slim.arg_scope(mobilenet_v1_arg_scope(is_training=False)):
        net_conv = mobilenet_v1_base(net_conv,
                                      ...
                                      reuse=reuse,
                                      scope=self._scope)
    if cfg.MOBILENET.FIXED_LAYERS < 16:
      with slim.arg_scope(mobilenet_v1_arg_scope(is_training=is_training)):
        net_conv = mobilenet_v1_base(net_conv,
                                      _CONV_DEFS[cfg.MOBILENET.FIXED_LAYERS:16],
...
# build the net - move final pooling to layer 16 instead of 12
